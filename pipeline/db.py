...
class Database:
    ...
    def get_active_shipments(
        ...
        sql = (
            "SELECT JS_UniqueConsignRef FROM cargowise_containers_new "
            "WHERE JS_TransportMode = %(mode)s AND primary_status = %(status)s "
            "AND (do_not_query IS NULL OR do_not_query != 1) AND show_shipment = 1 AND is_active = 1"
        )
        if extra_where:
            sql += f" AND ({extra_where})"
            logger.info(f"SQL ashish: {sql}")
    ...
    def mark_inactive(self, shipment_id: str) -> None:
        """Set is_active = false when CW says no business object."""
        conn = self._pool.connection()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE cargowise_containers_new "
                    "SET is_active = 0 WHERE JS_UniqueConsignRef = %s",
                    (shipment_id,),
                )
            conn.commit()
        finally:
