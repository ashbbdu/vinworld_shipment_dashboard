...
# )
# from pipeline.db import apply_eta_snapshot, apply_actual_snapshot
# from pipeline.cargowise import NoBusinessObjectError
...
#     except NoBusinessObjectError as e:
#         print(f"⚠️ [{shipment_id}] No business object found — marking is_active=false")
#         db.mark_inactive(shipment_id)
#         db.update_tracking(shipment_id, job_run_id, "completed", error=str(e))
#         return {"status": "completed", "shipment_id": shipment_id}
...
def process_shipment(shipment_id, job_run_id, db, cw_client, settings, retry_count=0):
    try:
        ...
        raw_milestones = extract_milestones_from_customized_fields(customized_fields, transport_mode, job_opened_date=None, trade_type=trade_type)
        record["milestones"] = json.dumps(raw_milestones)
        ...
        milestones_new = build_milestones(transport_mode, service_level, actuals, dates, trade_type=trade_type)
        milestones_reformed = build_milestones_reformed(transport_mode, service_level, customized_fields , job_opened_date, trade_type=trade_type)
        ...
        print(f"📌 [{shipment_id}] Deriving status...")
        primary_status, derived_statuses = derive_status(
            milestones_reformed, transport_mode, service_level, trade_type=trade_type
        )
